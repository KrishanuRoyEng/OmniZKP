...
export default function OmniZKPPage() {
  ...
  const [encryptedNetworkShards, setEncryptedNetworkShards] = useState<
    string[]
  >([]);
  ...
  const handleRegister = useCallback(async () => {
    ...
    if (!id) {
      addLog("error", "Identity generation failed");
      return;
    }
    setIdentity(id);
    setCredential(null);
    ...
    addLog(
      "success",
      "3 shards encrypted and distributed to decentralized Guardians",
    );
  }, [addLog]);
  ...
  const handleVerify = useCallback(
    async (scope: string) => {
      ...
      const profile = (
        Object.values(VERIFIERS) as Array<
          (typeof VERIFIERS)[keyof typeof VERIFIERS]
        >
      ).find((v) => v.scope === scope);
      if (!profile) return;

      if (profile.name === "Acko Insurance" && !emailData) {
        addLog("error", "[REJECTED] Policy Violation: Valid DKIM .eml attestation required.");
        return;
      }
      ...
      try {
        ...
        if (useHardwareEnclave && hardwareKeys) {
          addLog(
            "info",
            "[ENCLAVE] Requesting hardware signature for sensor payload...",
          );
          teeSignature = await signSensorDataWithHardware(
            hardwareKeys.privateKey,
            safeDist,
            safeTime,
          );
          addLog(
            "success",
            `[ENCLAVE] Hardware signature acquired: ${teeSignature.substring(0, 16)}...`,
          );
        }
        const t0 = performance.now();

        const result = verify_proof_js(
          BigInt(1000 + safeDist),
          BigInt(0),
          BigInt(1000),
          BigInt(safeTime),
          nullifier,
        ...
        if (result.includes("[VERIFIED]") && !result.includes("INVALID")) {
          ...
          addLog(
            "success",
            `Proof valid  ·  nullifier: ${nullifier.slice(0, 18)}…`,
          );
          const vc = {
  ...
  const handleRecover = useCallback(() => {
    ...
    if (rec) {
      ...
      addLog(
        "success",
        "Secret reconstructed via Shamir interpolation over GF(2⁸)",
      );
    } else {
  ...
  const runAutomatedE2ETests = async () => {
    ...
    try {
      ...
      addLog(
        "info",
        "[TEST 2] Testing Circuit Soundness (Missing TEE Signature)...",
      );
      ...
      if (!resultUnsigned.includes("INVALID"))
        throw new Error("Circuit accepted unsigned payload!");
      ...
      addLog(
        "info",
        "[TEST 3] Testing Circuit Completeness (Valid TEE & Location)...",
      );
      const resultValid = verify_proof_js(
        ...
        BigInt(10),
        nullifier,
        "0xmock_hardware_sig_test",
      );
      if (!resultValid.includes("[VERIFIED]"))
        throw new Error("Circuit rejected valid payload!");
      ...
      if (recovered !== testId.masterSecret)
        throw new Error("Recovered secret mismatch!");
      addLog(
        "success",
        "[PASS] Master secret fully reconstructed from shards.",
      );
